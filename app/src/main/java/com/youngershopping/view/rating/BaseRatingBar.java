package com.youngershopping.view.rating;
...
public class BaseRatingBar extends LinearLayout implements SimpleRatingBar {
    ...
    private void verifyParamsValue() {
        ...
        if (mEmptyDrawable == null) {
            mEmptyDrawable = ContextCompat.getDrawable(getContext(), R.mipmap.rating_empty);
        }

        if (mFilledDrawable == null) {
            mFilledDrawable = ContextCompat.getDrawable(getContext(), R.mipmap.rating_filled);
        }
     ...
     * the view which id is 3 also need to be rating_filled.
     */
