package com.youngershopping.view.stepview;
...
import com.youngershopping.view.stepview.components.StateItemDescription;
import com.youngershopping.view.stepview.components.StateItemNumber;
import com.youngershopping.view.stepview.listeners.OnStateItemClickListener;
import com.youngershopping.view.stepview.utils.FontManager;
...
public class StateProgressBar extends View {
    ...
    private static final String SUPER_STATE_KEY = "superState";
        ...
        private ArrayList<String> mStateDescriptionData_Top = new ArrayList<>();
    private ArrayList<String> mStateDescriptionData_Bottom = new ArrayList<>();
    ...
    private void drawCircles(Canvas canvas, Paint paint, int startIndex, int endIndex) {
        for (int i = startIndex; i < endIndex; i++) {
            canvas.drawCircle(mCellWidth * (i + 1) - (mCellWidth / 2),
                    (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mStateRadius, paint);
        }
    }
    ...
    private int getDesiredHeight() {
        if (mStateDescriptionData_Top.isEmpty()) {
            return (int) (2 * mStateRadius) + (int) (mSpacing);
        } else {
            return (int) (2 * mStateRadius) + (int) (1.3 * mStateDescriptionSize)
                    + (int) (1.3 * mStateDescriptionSize) + (int) (mSpacing)
                    - (int) (mDescTopSpaceDecrementer);  // mStageHeight = mCellHeight + ( 2 * description Text Size)
        }
//        + (int) (mDescTopSpaceIncrementer) + (int) (mDescTopSpaceIncrementer)
    }
    ...
    private void drawState(Canvas canvas) {
        drawStateDescriptionText_Top(canvas);
        ...
        drawStateDescriptionText_Bottom(canvas);
    ...
    private void drawBackgroundCircles(Canvas canvas) {
        ...
        if (mStateDescriptionData_Top.size() == 2){
            mBackgroundPaint.setColor(getResources().getColor(R.color.order_red));
        }else{
//            mBackgroundPaint.setColor(mBackgroundColor);
        }
    ...
    private void drawLines(Canvas canvas, Paint paint, int startIndex, int endIndex) {
        ...
        if (endIndex > startIndex) {
            ...
            canvas.drawLine(startX, (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, stopX,
                    (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, paint);
    ...
    private void drawLineToCurrentState(Canvas canvas) {
        ...
        canvas.drawLine(mStartCenterX, (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mEndCenterX,
                (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mForegroundPaint);
    ...
    private void animateToCurrentState(Canvas canvas) {
        ...
        if (mAnimEndXPos < mStartCenterX || mStartCenterX > mEndCenterX) {
            ...
            if (!mIsStateNumberDescending) {
                canvas.drawLine(mStartCenterX, (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mAnimEndXPos,
                        (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mForegroundPaint);
                canvas.drawLine(mAnimEndXPos, (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mEndCenterX,
                        (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mBackgroundPaint);
            } else {
                canvas.drawLine(mEndCenterX, (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer,
                        mEndCenterX - (mAnimEndXPos - mStartCenterX), (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mForegroundPaint);
                canvas.drawLine(mEndCenterX - (mAnimEndXPos - mStartCenterX),
                        (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mStartCenterX,
                        (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mBackgroundPaint);
            }
            ...
            if (!mIsStateNumberDescending)
                ...
                canvas.drawLine(mEndCenterX, (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mStartCenterX,
                        (mCellHeight / 2) + (int) (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer, mForegroundPaint);
    ...
    private void drawStateDescriptionText_Top(Canvas canvas) {
        ...
        if (!mStateDescriptionData_Top.isEmpty()) {
            ...
            for (int i = 0; i < mStateDescriptionData_Top.size(); i++) {
                if (i < mMaxStateNumber) {

                    innerPaintType = selectDescriptionPaint(mCurrentStateNumber, i);
                    innerPaintType.setFakeBoldText(false);
                    xPos = (int) (mNextCellWidth - (mCellWidth / 2));
                    yPos = (int) ((int) (1 * mStateDescriptionSize)
                            + mStateDescriptionSize - mSpacing - mDescTopSpaceDecrementer);//mSpacing = mStateNumberForegroundPaint.getTextSize()


                    canvas.drawText(mIsStateNumberDescending ? mStateDescriptionData_Top.get(mStateDescriptionData_Top.size() - 1 - i) : mStateDescriptionData_Top.get(i), xPos, yPos, innerPaintType);
    ...
    private void drawStateDescriptionText_Bottom(Canvas canvas) {

        int xPos;
        int yPos;
        Paint innerPaintType;

        if (!mStateDescriptionData_Bottom.isEmpty()) {

            for (int i = 0; i < mStateDescriptionData_Bottom.size(); i++) {
                if (i < mMaxStateNumber) {

                    innerPaintType = selectDescriptionPaint(mCurrentStateNumber, i);
                    innerPaintType.setFakeBoldText(false);
                    xPos = (int) (mNextCellWidth - (mCellWidth / 2));
                    yPos = (int) (mCellHeight + mStateDescriptionSize
                            + mStateDescriptionSize - mSpacing - mDescTopSpaceDecrementer
                            + mDescTopSpaceIncrementer + mDescTopSpaceIncrementer);//mSpacing = mStateNumberForegroundPaint.getTextSize()
                    canvas.drawText(mIsStateNumberDescending ? mStateDescriptionData_Bottom.get(mStateDescriptionData_Bottom.size() - 1 - i) : mStateDescriptionData_Bottom.get(i), xPos, yPos, innerPaintType);
    ...
    public void setStateDescriptionData_Top(String[] stateDescriptionData) {
        mStateDescriptionData_Top = new ArrayList<>(Arrays.asList(stateDescriptionData));

        resolveStateDescriptionDataSize(mStateDescriptionData_Top);

        requestLayout();
    }

    public void setStateDescriptionData_Bottom(String[] stateDescriptionData) {
        mStateDescriptionData_Bottom = new ArrayList<>(Arrays.asList(stateDescriptionData));

    }

    public void setStateDescriptionData_Top(ArrayList<String> stateDescriptionData) {
        mStateDescriptionData_Top = stateDescriptionData;
    ...
    public void setStateDescriptionData_Bottom(ArrayList<String> stateDescriptionData) {
        mStateDescriptionData_Bottom = stateDescriptionData;

    }

    public List<String> getStateDescriptionData() {
        return mStateDescriptionData_Top;
    }
    ...
    private void drawStateNumberText(Canvas canvas, int noOfCircles) {
        ...
        for (int i = 0; i < noOfCircles; i++) {
            ...
            yPos = (int) ((int) ((mCellHeight / 2) - ((innerPaintType.descent() + innerPaintType.ascent()) / 2)) + (1.3 * mStateDescriptionSize) + mDescTopSpaceIncrementer);
